...
static void menu_texture(menu_t *mn)
{
    ...
    sfTexture *map_tx, *button_tx, *cursor_tx;
    sfIntRect c_size = {25, 0, 60, 84};
    sfVector2u cursor_size = {75, 103};
    sfVector2u hotspot = {0, 0};
    const sfUint8 *cursor;

    map_tx = sfTexture_createFromFile("assets/textures/menu_bg.jpg", NULL);
    cursor_tx = sfTexture_createFromFile("assets/textures/menu_ss.png", \
                                                                    &c_size);
    cursor = sfImage_getPixelsPtr(sfTexture_copyToImage(cursor_tx));
    mn->cursor = sfCursor_createFromPixels(cursor, cursor_size, hotspot);
    for (int i = 0; i < NB_BUTTON; i++) {
