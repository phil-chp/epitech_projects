...
void    CommunicationAPI::sendMessage(std::string const &userName, std::string const &message) const
{
    if (!hasUser(userName))
...
void    CommunicationAPI::receiveMessage(std::string const &userName, std::string &message) const
{
    if (!hasUser(userName))
        throw std::invalid_argument("User " + userName + " not found.");
    if (_missionName.size() == 0)
