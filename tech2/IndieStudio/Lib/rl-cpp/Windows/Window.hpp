...
*/
    ...
    #include "Camera/CameraModes.hpp"
    #include "Colors/Colors.hpp"
    #include "Collisions/Ray/Ray.hpp"
    #include <iostream>
    #include <string>
...
namespace rl {
    ...
    class Window {
        private:
            Window(const std::string &title = "Bomberman", int width = DEFAULT_WIDTH, int height = DEFAULT_HEIGHT)
                : _title(title)
                , _width(width)
                , _height(height)
                , _isOpen(false)
                , _backgroundColor(RL_WHITE)
                , _cursor()
                , _fps(60)
                , _camera2D()
                , _camera3D()
        ...
        // WINDOW GETTERS ----------------------------------------------- //
            ...
            void beginShaderMode(const Shader &shader) const;
