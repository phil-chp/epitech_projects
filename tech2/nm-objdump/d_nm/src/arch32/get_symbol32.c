...
static char get_symbol_info(char c, Elf32_Sym symtab)
{
    if (c != '?') return (c);
...
static char get_symbol_section(char c, Elf32_Sym symtab)
{
    if (c != '?') return (c);
...
static char get_symbol_shared_type(char c, Elf32_Sym symtab, Elf32_Shdr *shared)
{
    if (c != '?') return (c);
    ...
    &&         shared[symtab.st_shndx].sh_flags == SHF_ALLOC) {
        ...
        uint32_t alloc_or_execinstr = (SHF_ALLOC | SHF_EXECINSTR);
...
static char get_symbol_program_header(char c, Elf32_Sym symtab,
                                        Elf32_Phdr *program)
{
    if (c != '?') return (c);
...
char get_symbol32(Elf32_Sym symtab, Elf32_Shdr *shared, Elf32_Phdr *program)
{
    ...
    if (c != '?' && ELF32_ST_BIND(symtab.st_info) == STB_LOCAL)
        c += 32;
    return (c);
}
