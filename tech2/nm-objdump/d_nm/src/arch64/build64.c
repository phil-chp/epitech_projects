...
static char *init_name(elf64_t *nm, data64_t *data, size_t i, void *name_offset)
{
    size_t name_size;
                ...
                size_t symtab_index, void *name_offset)
{
    ...
    nm->data_size = nm->shared[symtab_index].sh_size / sizeof(Elf64_Sym);
    for (size_t i = 0; i < nm->data_size; ++i) {
        if (strlen((char *)&nm->symtab[i].st_name) == 0
        ||  get_symbol64(nm->symtab[i], nm->shared, nm->program) == 'a') ++n;
    }
    (*data) = malloc(sizeof(data64_t) * (nm->data_size - n));
    for (size_t i = 0; i < nm->data_size; ++i) {
        if (strlen((char *)&nm->symtab[i].st_name) == 0
        ||  get_symbol64(nm->symtab[i], nm->shared, nm->program) == 'a') {
            continue;
        }
        (*data)[j].symbol = \
            get_symbol64(nm->symtab[i], nm->shared, nm->program);
